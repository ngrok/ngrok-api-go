...
func (x *EndpointPolicies) String() string {
	return x.GoString()
}

func (x *EndpointPolicies) GoString() string {
	var b bytes.Buffer
	fmt.Fprintf(&b, "EndpointPolicies {\n")
	tw := tabwriter.NewWriter(&b, 0, 4, 0, ' ', 0)
	fmt.Fprintf(tw, "\tEnabled\t%v\n", x.Enabled)
...
type HTTPSEdgeRouteCreate struct {
	...
	// policies module configuration or null
...
type EdgePoliciesReplace struct {
	ID     string           `json:"id,omitempty"`
	Module EndpointPolicies `json:"module,omitempty"`
}

func (x *EdgePoliciesReplace) String() string {
	return fmt.Sprintf("EdgePoliciesReplace{ID: %v}", x.ID)

}

func (x *EdgePoliciesReplace) GoString() string {
	var b bytes.Buffer
	fmt.Fprintf(&b, "EdgePoliciesReplace {\n")
	tw := tabwriter.NewWriter(&b, 0, 4, 0, ' ', 0)
	fmt.Fprintf(tw, "\tID\t%v\n", x.ID)
	fmt.Fprintf(tw, "\tModule\t%v\n", x.Module)
	tw.Flush()
...
type EdgeRoutePoliciesReplace struct {
	EdgeID string           `json:"edge_id,omitempty"`
	ID     string           `json:"id,omitempty"`
	Module EndpointPolicies `json:"module,omitempty"`
}

func (x *EdgeRoutePoliciesReplace) String() string {
	return fmt.Sprintf("EdgeRoutePoliciesReplace{ID: %v}", x.ID)

}

func (x *EdgeRoutePoliciesReplace) GoString() string {
	var b bytes.Buffer
	fmt.Fprintf(&b, "EdgeRoutePoliciesReplace {\n")
	tw := tabwriter.NewWriter(&b, 0, 4, 0, ' ', 0)
	fmt.Fprintf(tw, "\tEdgeID\t%v\n", x.EdgeID)
	fmt.Fprintf(tw, "\tID\t%v\n", x.ID)
	fmt.Fprintf(tw, "\tModule\t%v\n", x.Module)
